...
**Easier**
- Feature parity with the TUI by construction — anything a human can do in claude, june15 can do.
- OAuth login, permission prompts, and trust dialogs work without a separate code path.
- june15 has zero compile-time coupling to Anthropic's private SDK; only the user-facing CLI behavior is load-bearing.
...
- Parsing a rendered TUI is fragile. We mitigate via:
  ...
  - Versioned compatibility: each release pins the `claude` versions it has been verified against and ships a `june15 doctor` command that flags untested binary versions.
...
- **Direct use of `@anthropic-ai/claude-agent-sdk`** — Means depending on a private IPC contract that Anthropic does not promise to keep stable across releases of the SDK and CLI. Also re-exports the SDK's opinionated shape (`StreamingCallbacks`-style); june15 should own its own HTTP contract.
