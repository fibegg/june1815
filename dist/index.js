...
import { z } from 'zod';
import { existsSync, readdirSync, accessSync, constants, readFileSync, rmSync, mkdirSync, writeFileSync } from 'fs';
...
function loggerOptionsFromConfig(config, isStdoutTty) {
  const effectiveMode = config.mode ?? (isStdoutTty ? "interactive" : "headless");
...
var ANSI_RE = /\x1b(?:\[[0-9;?]*[ -/]*[@-~]|\][^\x07]*\x07|[78=>]|\([AB012])/g;
var realProbeSpawn = {
  run: (cmd, args, timeoutMs) => new Promise((resolve) => {
...
var ANSI_RE3 = new RegExp(ANSI_PATTERN, "g");
function stripAnsi2(s) {
  return s.replace(OSC_RE, "").replace(ANSI_RE3, "");
}
function stripAnsiLines(lines) {
  return lines.map(stripAnsi2);
}
